...
**What it keeps.** One record of 83 words in memory that survives every reset but power loss (ESP8266 user RTC words 32–114, words 0–31 being the bootloader's; ESP32 `RTC_NOINIT_ATTR`, lost on an EN-pin reset): a magic, layout and flags word, a CRC32 over the body, the build id, the boot sequence and EventBus drop count, a phase marker, the last tick's uptime and heap minimum, a fast ring (16 samples, one every 10 s: free heap and largest block, 16-byte units) and a slow ring (8 samples, one every 10 min), the failed-allocation group (OBS-4: a count word validated by its complement, the last failure's size, free heap and largest block at that moment, its site — the caller on ESP8266, the requested caps on ESP32 — and its uptime; outside the CRC like the phase marker, because it is stored straight to RTC from the failing task), the exception registers and sixteen stack words the crash callback copied. Layout 2 since OBS-4; a layout-1 record is read with its own CRC rule once and rewritten.
...
**Sampling cost.** Free heap is read every `Core::loop()` (a running minimum between ticks); the largest free block only at tick time plus at most one extra read per interval when the minimum has fallen by more than `heapCliffThresholdBytes()` or an allocation failed (one gate for both) — on ESP8266 that read walks the heap with interrupts off.

**Survived allocation failures (OBS-4).** On ESP32 the recorder takes the single `heap_caps_register_failed_alloc_callback()` slot as the last act of `begin()`; the hook runs on whichever task failed, on either core, and writes the group under a critical section with no log and no allocation. On ESP8266 there is no hook: every millisecond-gated sample takes the core's `umm_last_fail_alloc_addr/size` into the group and clears them, so the crash callback's fail fields mean "the allocation that killed us, or nothing", and the core's own postmortem line `last failed alloc call` reports the same narrower thing. **What a stock ESP8266 build can count**: `operator new` (including `nothrow`) and newlib's internal allocations (`strdup`, the `printf` family, with the libc routine as the site). `String`, ArduinoJson, lwIP and the SDK allocate through umm's own `malloc`/`realloc`/`pvPortMalloc`, which record nothing unless the build carries `-DDEBUG_ESP_OOM` — the diagnostic profile below. `Core::loop()` logs `Allocation failures since boot: N, last S B` at most once a minute; `bootdiag` prints the promoted record's group and a `this boot:` line for the current run.
...
| `noteFailedAlloc(size, site)`, `onFailedAlloc(FailedAllocHook)`, `failedAllocCount()`, `lastFailedAllocSize()`, `failedAllocHookInstalled()` | The failed-allocation group: fed by the ESP32 hook or the ESP8266 latch, a user hook chained after ours, the current run's count for the loop's warning line |
| `format(char*, size_t)` | The promoted record as text, lines under 128 characters, oldest ring sample first (the `bootdiag` command's first block); allocation-free but for the reset reason's name, which the platform returns as a `String` |

**Defines.** `DOMOTICS_CRASH_HOOKS` (default `1`): defines the ESP8266 `custom_crash_callback` — a strong symbol, so a sketch that defines its own must set `-DDOMOTICS_CRASH_HOOKS=0` or fail at link — the ESP32 shutdown handler, and the ESP32 failed-allocation hook (a single slot IDF replaces silently: a sketch with its own registers it after `Core::begin()` or chains through `onFailedAlloc()`). `DOMOTICS_FLIGHT_RECORDER_TICK` (default `1`, `0` compiles the sampler and the marker out; promotion and the crash record stay) and `DOMOTICS_FLIGHT_RECORDER_PHASE` (default `1`, `0` compiles the marker out alone): the two knobs on the per-loop cost, measured at about 7 µs per idle loop on an ESP8266 (marker ≈ 4, heap read ≈ 3). `DOMOTICS_BUILD_ID='"<string>"'` (quotes included): a `.rodata` string `strings firmware.elf` finds, logged at boot, its CRC32 in the record; without it the compile date and time stand in, so two firmwares still get two ids. `DOMOTICS_ENABLE_CRASH_COMMANDS`: `System` registers the `crash` console command; test environments only. CI compiles FullStack with the three opt-outs on every target.

**Platform seams** (`Platform_*.h`): `rtcRead`/`rtcWrite`/`rtcStoreWord`/`rtcWordsAvailable`, `getAllocatableFreeHeap` (ESP32: `heap_caps` with `MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL`, never `ESP.getFreeHeap()`, which counts IRAM `malloc` cannot use), `getLargestFreeBlock`, `heapCliffThresholdBytes`, `installRestartHook`, `crashForTest` (`abort|oom|null|swdt|hwdt|hang|restart`, and the survived `nothrow|squeeze|release` since OBS-4), and OBS-4's `installFailedAllocHook`/`supportsFailedAllocHook`, `takeLastFailedAlloc`, `failGroupEnter/Leave`, `getMillisAnyContext` (ESP32: `esp_timer_get_time`, IRAM where `millis()` is not). The stub keeps RTC in a RAM array cleared only by `clearRtcForTest()`, scripts the ESP8266 globals and fires the ESP32 hook on demand, so the native suite drives every rule.

**The ESP8266 diagnostic profile (OBS-4).** `[env:nodemcuv2-diag]` in `tools/on-device/probes/obs-lotb-probe/platformio.ini` is the recipe: `build_flags` (they must reach every library) `-DDEBUG_ESP_OOM -g`, optionally `-DDEBUG_ESP_HWDT` or `-DDEBUG_ESP_HWDT_NOEXTRA4K`. Measured on a nodemcuv2 (the probe, 2026-09-06):

| flag | what it buys | cost against the stock build |
|---|---|---|
| `-g` | file and line from `addr2line` on the kept ELF — the callback's `epc1` and fail caller, the group's site (`String::changeBuffer`, `WString.cpp:246`, for a `String` that failed to grow) | none in RAM; the ELF grows; `firmware.bin` is **not** byte-identical (`.irom0.text` −416 B, cause not chased) |
| `-DDEBUG_ESP_OOM` | the C paths record: `String`, ArduinoJson, lwIP and SDK failures reach the recorder's group (on the stock build only `new` and newlib's own do); `:oom(size)@file:line` on the console when `Serial.setDebugOutput(true)` | +1 316 B flash, **+840 B IRAM**, +44 B RAM; loop cost inside the boot-to-boot spread |
| `-DDEBUG_ESP_HWDT` / `_NOEXTRA4K` | a stack dump on the boot after a hardware watchdog (372 lines on the probe), for the exception decoder | +1 384 B flash, +212 B IRAM, −48 B heap at boot; a greeting at the ROM's 74 880 baud on every boot |

Figures and the board sequence: the OBS-4 entry of `docs/CODE-ROADMAP.md`.
