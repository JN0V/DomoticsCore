...
Defined in `DomoticsCore/FlightRecorder.h` (`DomoticsCore::FlightRecorder`, one process-wide instance via `instance()`); the RTC storage and the platform hooks are defined once in `src/FlightRecorder.cpp`.

**What it keeps.** One record of 83 words in memory that survives every reset but power loss (ESP8266 user RTC words 32–114, words 0–31 being the bootloader's; ESP32 `RTC_NOINIT_ATTR`, lost on an EN-pin reset): a magic, layout and flags word, a CRC32 over the body, the build id, the boot sequence and EventBus drop count, a phase marker, the last tick's uptime and heap minimum, a fast ring (16 samples, one every 10 s: free heap and largest block, 16-byte units) and a slow ring (8 samples, one every 10 min), the failed-allocation group, the exception registers and sixteen stack words the crash callback copied. Two words are outside the CRC because they are stored straight to RTC between flushes and validate themselves by a complement: the phase marker, and the count word of the failed-allocation group. The layout is versioned; a record of the previous layout (which had no group) is read once with its own CRC rule and rewritten.
...
**Sampling.** Free heap is read every `Core::loop()`, at most once per millisecond (a running minimum between ticks); the largest free block only at tick time plus at most one extra read per interval, when the minimum has fallen by more than `heapCliffThresholdBytes()` or an allocation failed — one gate for both, because on ESP8266 that read walks the heap with interrupts off.

**The failed-allocation group** records the last allocation that failed and was survived: a count word (`count << 16 | ~count`, saturating at 65 535), the size, free heap and largest block at that moment in 16-byte units, the site, and the uptime. It is written to RTC at once, count word last, so a death in the next microsecond still finds it. On ESP32 the recorder takes the single `heap_caps_register_failed_alloc_callback()` slot as the last act of `begin()`; the hook runs on whichever task failed, on either core, and writes the group under a critical section with no log and no allocation; the site is the requested capabilities word. On ESP8266 there is no hook: every millisecond-gated sample takes the core's `umm_last_fail_alloc_addr/size` into the group and clears them, so the crash callback's fail fields mean "the allocation that killed us, or nothing" — a fatal `new` sets them and aborts with no sample in between; a survived failure that reaches the callback under an exception in that same millisecond is routed to the group. The core's own postmortem line `last failed alloc call` therefore reports a failure since the last sample, not since boot. The site is the failing caller. **What a stock ESP8266 build can count**: `operator new` (including `nothrow`) and newlib's internal allocations (`strdup`, the `printf` family, with the libc routine as the site). `String`, ArduinoJson, lwIP and the SDK allocate through umm's own `malloc`/`realloc`/`pvPortMalloc`, which record nothing unless the build carries `-DDEBUG_ESP_OOM` — the diagnostic profile below. `Core::loop()` logs `Allocation failures since boot: N, last S B` at most once a minute; `bootdiag` prints the promoted record's group before the ring and a `this boot:` line for the current run. While a promoted death is held in RTC, the current run's group stays in RAM until `acknowledge()`, so the held death keeps its own. On a healthy FullStack the hook does not fire — none in ten minutes idle, through an OTA upload, or through a WebUI session with MQTT connected — so one firing is news; a device squeezed to 12 KB of allocatable heap fires within seconds and dies of the task watchdog minutes later, with the count and the last failure on the record.

| Method | Purpose |
|---|---|
| `begin(bool holdUntilAcknowledged)` | Read RTC, decide, start the fresh record or hold the promoted one; then take the heap hook |
...
| `tick()` | Every loop: heap minimum and the ESP8266 latch; every 10 s: a sample and a flush |
...
| `noteFailedAlloc(size, site)` | A survived failure, from the ESP32 hook or the ESP8266 latch: the group to RTC at once |
| `onFailedAlloc(FailedAllocHook)` | Chain a user hook after the group is written; it runs in the failing task's context, inside the allocator — no allocation, no log, no blocking |
| `failedAllocSnapshot(count, lastSize)`, `failedAllocCount()`, `lastFailedAllocSize()`, `failedAllocHookInstalled()` | The current run's group, the snapshot read under the section |
| `format(char*, size_t)` | The promoted record as text, lines under 128 characters, the group before the ring, oldest ring sample first (the `bootdiag` command's first block); allocation-free but for the reset reason's name, which the platform returns as a `String` |

**Defines.** `DOMOTICS_CRASH_HOOKS` (default `1`): defines the ESP8266 `custom_crash_callback` — a strong symbol, so a sketch that defines its own must set `-DDOMOTICS_CRASH_HOOKS=0` or fail at link — the ESP32 shutdown handler, and the ESP32 failed-allocation hook (a single slot IDF replaces silently: a sketch with its own registers it after `Core::begin()` or chains through `onFailedAlloc()`); with the define at `0` the hooks are absent by design and nothing warns. `DOMOTICS_FLIGHT_RECORDER_TICK` (default `1`, `0` compiles the sampler, the ESP8266 latch and the marker out; promotion and the crash record stay — `Core::begin()` says so where the platform has no hook) and `DOMOTICS_FLIGHT_RECORDER_PHASE` (default `1`, `0` compiles the marker out alone) are the two knobs on the per-loop cost. On ESP8266 that cost cannot be separated from the link layout: an idle `System::loop()` reads 39 µs with the sampler compiled out and anywhere from 46 to 88 µs with it in, across builds of one source that differ only in where the linker put the code — the flash instruction cache decides, not the sampler. `DOMOTICS_BUILD_ID='"<string>"'` (quotes included): a `.rodata` string `strings firmware.elf` finds, logged at boot, its CRC32 in the record; without it the compile date and time stand in, so two firmwares still get two ids. `DOMOTICS_ENABLE_CRASH_COMMANDS`: `System` registers the `crash` console command; test environments only. CI compiles FullStack with the three opt-outs on every target.

**Platform seams** (`Platform_*.h`): `rtcRead`/`rtcWrite`/`rtcStoreWord`/`rtcWordsAvailable`, `getAllocatableFreeHeap` (ESP32: `heap_caps` with `MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL`, never `ESP.getFreeHeap()`, which counts IRAM `malloc` cannot use), `getLargestFreeBlock`, `heapCliffThresholdBytes`, `installRestartHook`, `installFailedAllocHook`/`supportsFailedAllocHook`, `takeLastFailedAlloc`, `failGroupEnter/Leave`, `getMillisAnyContext` (ESP32: `esp_timer_get_time`, IRAM where `millis()` is not), `crashForTest` (`abort|oom|null|swdt|hwdt|hang|restart` die; `nothrow|squeeze|release` return — `crashKindSurvives()` tells them apart) . The stub keeps RTC in a RAM array cleared only by `clearRtcForTest()`, scripts the ESP8266 globals and fires the ESP32 hook on demand, so the native suite drives every rule.

**The ESP8266 diagnostic profile.** `[env:nodemcuv2-diag]` in `tools/on-device/probes/obs-lotb-probe/platformio.ini` is the recipe: `build_flags` (they must reach every library) `-DDEBUG_ESP_OOM -g`, optionally `-DDEBUG_ESP_HWDT` or `-DDEBUG_ESP_HWDT_NOEXTRA4K`. Measured on a nodemcuv2 against the stock build of the same source:

| flag | what it buys | cost |
|---|---|---|
| `-g` | file and line from `addr2line` on the kept ELF — the callback's `epc1` and fail caller, the group's site (`String::changeBuffer`, `WString.cpp:246`, for a `String` that failed to grow) | none in RAM; the ELF grows; `firmware.bin` is **not** byte-identical (`.irom0.text` −416 B, the linker folding identical functions differently once debug info tells them apart) |
| `-DDEBUG_ESP_OOM` | the C paths record: `String`, ArduinoJson, lwIP and SDK failures reach the recorder's group (on the stock build only `new` and newlib's own do); `:oom(size)@file:line` on the console when `Serial.setDebugOutput(true)` | +1 316 B flash, **+840 B IRAM**, +44 B RAM; loop cost inside the layout spread above |
| `-DDEBUG_ESP_HWDT` / `_NOEXTRA4K` | a stack dump on the boot after a hardware watchdog (372 lines on the probe), for the exception decoder | +1 384 B flash, +212 B IRAM, −48 B heap at boot for either variant; a greeting at the ROM's 74 880 baud on every boot |

History and the board measurements behind these figures: `docs/CODE-ROADMAP.md`.
...
- **Publishing from another task is safe.** The queue, the sticky store and the pending-per-topic map move under a recursive lock (`HAL::Platform::RecursiveLock` — a statically allocated FreeRTOS mutex on ESP32, nothing at all on ESP8266, which has one execution context). This is what an ESP32 web handler does: it runs on the async server's task while `Core::loop()` drains on the application task. On ESP32 the lock costs about 1.3 KB of flash for the whole library and no static RAM, and a publish pays one take and one give; on ESP8266 the image is unchanged and the bus grows four bytes.
- **Handlers run outside the lock.** `poll()` takes the queue, releases, then dispatches, so a slow handler never holds a publisher on another task behind it. A handler that publishes re-enters through the same recursive lock.
- **Subscription belongs to the loop.** `subscribe()`, `unsubscribe()` and `unsubscribeOwner()` must NOT be called during `poll()` dispatch — an `assert` guards this in debug builds — and the handler list is read during dispatch without the lock, so subscribing from another task is not supported.
