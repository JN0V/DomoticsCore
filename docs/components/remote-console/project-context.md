...
- **Bounded client input** (BUG-22): `handleClient()` reads at most 512 bytes per `loop()` call, so a client streaming without pause cannot keep the loop from returning, and the command buffer is capped at 128 bytes, so one that never sends a newline cannot grow it without end. Both bounds are needed: the read bound alone protects the loop while leaving the heap open to a slow, endless line. Input beyond the cap is dropped, which is where the line cleaner already truncated it.
