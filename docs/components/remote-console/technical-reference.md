...
> **WARNING -- Unenforced config fields (security risk):**
> The fields `requireAuth`, `password`, and `allowCommands` are **declared in the struct but NOT enforced** by the current implementation. Setting `requireAuth = true` does **not** enable any authentication -- clients can connect and execute commands without a password. Setting `allowCommands = false` does **not** prevent command execution -- all commands remain accessible. These fields exist in the config struct for forward-compatibility but have **no effect** at runtime. Do not rely on them for security. Use `allowedIPs` (which IS enforced) or network-level firewalling instead.

```cpp
struct RemoteConsoleConfig {
    bool enabled = true;
    uint16_t port = 23;
    bool requireAuth = false;              // NOT ENFORCED -- see warning above
    String password = "";                  // NOT ENFORCED -- see warning above
    uint32_t bufferSize = DOMOTICS_LOG_BUFFER_SIZE;
    bool allowCommands = true;             // NOT ENFORCED -- see warning above
...
| `requireAuth`    | `bool`                        | `false`                      | **NOT ENFORCED.** Declared but never checked -- authentication is not implemented. |
| `password`       | `String`                      | `""`                         | **NOT ENFORCED.** Declared but never checked -- no authentication logic exists.    |
...
| `allowCommands`  | `bool`                        | `true`                       | **NOT ENFORCED.** Declared but never checked -- commands are always accepted.      |
| `allowedIPs`     | `std::vector<HAL::IPAddress>` | `{}` (empty = all allowed)   | IP whitelist. An empty vector permits all IPs. **This IS enforced.**               |
...
| `void loop()`                                           | Accepts new clients, handles input from existing clients, and cleans up disconnected clients. |
