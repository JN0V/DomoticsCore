...
- **Problem**: every payload constant the panel publishes — `pl_arm_home`:
  `ARM_HOME`, `pl_disarm`: `DISARM`, five more — is the value Home Assistant
  already uses when the key is absent. Measured: **188 characters at six arm
  modes**, 46 at one. The document states what the other side assumes and, until
  BUG-63, omitted the three keys where the two sides disagree. It was exactly the
  wrong way round.
- **Consequence, measured**: this is the headroom a code-less panel lacks. At six
  arm modes such a panel is 726 characters against the 699-character field and is
  refused, entity and all; without the redundant keys it would be 538. BUG-63's
  lot left them in place deliberately — the HIGH had to land — and parked this.
- **Not a free removal, which is why it is a decision and not a fix.** The keys
  are the only place the device states the vocabulary it accepts. Omit them and
  the agreement holds by construction today, because `AlarmPanelCommand::*` *is*
  Home Assistant's documented default; the day either side moves, the device
  receives a payload the consumer's comparison does not match and nothing says
  so. That asymmetry is the argument for doing BUG-63 first and this separately:
  an absent `cod_arm_req` is a certain failure now, an absent `pl_arm_away` is a
  risk later.
- **The arbitration**: drop them (188 characters back, a written reliance on
  Home Assistant's defaults); keep them and state the ceiling (today's answer,
  with the panel's ceiling 188 characters lower than it need be); or emit them
  only where a caller has overridden a payload, which needs an override to exist
  and none does.
- **Refs**: BUG-38, which abbreviated every key for this same budget; BUG-63,
  whose lot measured it.
...
### TEST-12 — RemoteConsole declares no device environment, so its platform halves are compiled and never run [MEDIUM] — **NEW (2026-09-24, filed by BUG-64's lot)**

- **Files**: `DomoticsCore-RemoteConsole/platformio.ini`, whose only environment is
  `[env:native]`, against `CoreLog_ESP32.h` and `CoreLog_ESP8266.h`.
- **Problem**: the two platform mechanisms BUG-64 added reach every target through
  `Logger.h`, so the required cross-compilation checks build them — and nothing
  executes them. The component has no `esp8266dev` or `esp32dev` environment, so
  it cannot appear in `Build on-device suites` either, and the evidence for both
  halves is a pair of hand-run probes whose figures live in a roadmap entry.
- **What that leaves unverified**: that `installCapture()` takes the sink and
  `removeCapture()` gives it back; that a provoked core line reaches
  `getRecentLogs()`; that the ESP8266 branch of the `core` command switches
  anything; that the intake's bounds hold with the real `MAX_LINE`; and that the
  new 20-entry ring still fits beside WiFi and MQTT on a nodemcuv2.
...
### CI-19 — HomeAssistant's `esp32dev` test environment cannot compile the suite it declares [LOW] — **NEW (2026-09-23, by BUG-46's lot)**
...
- **Fix**: add `test_ha_component` to that environment's `test_ignore`, or gate
  the stub calls so the suite compiles for a board. The sibling of CI-11, from
  the other direction — that one declares an environment with no suite, this one
  declares a suite the environment cannot build.
...
| **Total** | **191 items** | | **0C, 0H, 28M, 56L** (120 resolved) |
