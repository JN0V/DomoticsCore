...
- **Files**: `Platform_ESP32.h` (`DOMOTICS_LOG_BUFFER_SIZE`), `RemoteConsole.h`
  (`log()`, the history's `std::vector<LogEntry>`).
- **Filed as a leak and corrected the next morning**: on a production ESP32, free
  allocatable heap fell -639 B/h over the first 4.8 h after the 2.6.1 → 2.7.0
  upgrade, then flattened — -231, -196, -112, then 0 B/h over five samples at
  85 064, against 107-109 KB on 2.6.1. A curve converging on a floor, not a line;
  `allocation_failures` stayed 0. What survived was the footprint.
- **Cause, found in this repository's own record**: BUG-64's lot moved the
  console's history from **50 to 150 entries on ESP32** (and 5 to 20 on ESP8266)
  and wrote the cost into its entry — *"23 KB when full"* — without arguing the
  number, and 2.7.0's CHANGELOG never mentioned it. The history fills as lines
  arrive, which is the settling curve; a structure reaching its cap, exactly as
  this entry's own "honest limits" had warned.
- **Measured through the component on the WROOM-32D**, allocatable heap:
  ```
  line      50 (2.6.1)   150 (2.7.0)   64 (now)
  40 ch        5 784        19 856        6 672
  80 ch        8 176        27 056        9 744
  120 ch       9 776        31 856       11 792
  ```
  150 − 50 at 120 characters is **22 080 B**, the production gap. Past the cap
  nothing moves. The step at the 129th line was the vector doubling to 256 slots:
  106 dead, 4.2 KB, because nothing reserved.
- **Fixed**: the ESP32 default is **64**, more than 2.6.1's 50 because platform
  lines now share the buffer with ours; ESP8266 stays at 20, which BUG-64 argued
  against a measured SDK burst. The slots are reserved once, at the cap, by the
  first line — `clearBuffer()` releases them, so a reservation made in `begin()`
  alone would have been lost at the first clear.
- **Verification**: two native cases, each red without the reservation
  (`Expected 100 Was 128`, `Expected 100 Was 1`); a device case that budgets the
  default history at 16 KB and fails on 2.7.0's default at `Expected 30000 to be
  less than or equal to 16384` — the release would have been stopped by it. 9/9
  on the WROOM-32D, 8 and one skip on the nodemcuv2.
- **The 2.7.0 CHANGELOG entry now says what it omitted**, with the workaround a
  deployed device has today: `RemoteConsoleConfig::bufferSize = 50`. **A line is
  owed at the next release**: the ESP32 default goes from 150 to 64.
- **Note for anyone measuring this**: `ESP.getFreeHeap()` and
  `getAllocatableFreeHeap()` differ by about 44 KB on this chip — the former
  counts 32-bit-only IRAM. Every figure here is the allocatable one.
- **Refs**: filed from a production ESP32 running 2.7.0 since 2026-09-24; BUG-64,
  whose lot made the change; the probe `tools/on-device/probes/console-ring-cost`.
