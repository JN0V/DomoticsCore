...
### SEC-16 — OTA: the two state-changing routes are gated by CSRF alone, not by authentication [HIGH] — **NEW (2026-09-21, demonstrated on the WROOM-32D)**
...
- **Fix**: `if (webui->getConfig().enableAuth && !webui->authorize(request))
  { request->requestAuthentication(); return; }` after each `checkCsrf`, which
  is what the upload route next door already does. One line each, plus a native
  case per route and a board pair with and without credentials.
...
| **Total** | **181 items** | | **0C, 1H, 28M, 53L** (112 resolved) |
