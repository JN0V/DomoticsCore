...
### BUG-67 — RemoteConsole: on ESP32 a console started before the network stack aborts the device [MEDIUM] — **NEW (2026-09-24, filed by TEST-12's lot)**
...
- **Reachable how**: the shipped stack registers WiFi first, so `FullStack` and
  ...
  makes ordinary.
- **The fix is a decision, which is why it is filed rather than done**: the
  server can open lazily in `loop()` once the stack answers, or `begin()` can
  refuse to open it and say so. Either moves when a port starts listening, which
  is a behaviour change with its own red-then-green on both boards.
- **Refs**: TEST-12, whose suite found it and works around it in one line;
  CI-14, the other case of a component that compiles and cannot run.
...
- **What the first flash found, before any assertion ran.** On ESP32 the
  ...
  waiting for Unity output from a firmware that was restarting. The suite puts
  the radio in station mode and joins nothing.
- **Verification**: 7/7 on the nodemcuv2 and on the WROOM-32D. With the
  platform's sink not taken, five of seven fail on each; on ESP32 the capture
  ...
  environments, and four deprecated `ICACHE_RAM_ATTR` declarations the build had
  been warning about at every use are now `IRAM_ATTR`.
...
- **The job still lists its projects, deliberately.** Measured while closing
  this: five components declare `esp8266dev` and carry a `test/`, while the job
  runs ten entries — two of them nested projects (`WebUI/test/test_schema_memory`,
  `FullStack`) that no such rule discovers, and three named ESP32 environments.
  Discovery would cover half the run and would lose what the list gives, which
  is failing loudly when a listed project disappears.
...
| **Total** | **192 items** | | **0C, 0H, 26M, 55L** (124 resolved) |
