...
### BUG-62 — HomeAssistant: every MQTT message that is not its own is reported at error level [LOW] — **NEW (2026-09-23, filed from AlarmControl's bench on 2.6.1)**
...
### SIZE-5 — test_ha_component.cpp (1 355 code lines) [MEDIUM] — **re-measured 2026-09-21**

- **The worst breach in the tree**: 2 010 lines raw, **1 355** excluding blanks
  and comments — 70 % over the limit, and it grew while the entry sat open.
...
| **Total** | **188 items** | | **0C, 1H, 28M, 56L** (116 resolved) |

The severity columns sum across the rows. **It went back to one on
...
repository's bench. Twice before, the column was refilled by a board run of
...
2026-09-21 left on its pre-triage figures, re-derived here — and the LOW column to 53:
