...
### BUG-72 — NTP: `setConfig()` follows `enabled` one way only [LOW] — **NEW (2026-09-25, filed by BUG-69's review)**

- **Files**: `NTP.h` (`setConfig()`), reached from `NTPWebUI.h`'s `enabled` field.
- **Problem**: `setConfig()` restarts the client when a restart-worthy field
  changes *and* the new config is enabled. Turning `enabled` off therefore stops
  nothing: `loop()` returns early, so no event and no statistic moves, but the
  SNTP client keeps setting the clock until the next boot, which reads the
  persisted `false` and never starts it.
- **Consequence**: unticking "Enable NTP Sync" in the settings card is stored and
  reported as applied, and the device keeps syncing its time from the network.
- **Also**: the restart calls `begin()` whatever `shutdown()` said, so an
  application that calls `setConfig()` on a stopped component starts it again —
  the gate BUG-67's review found at the console. The WebUI cannot reach this half:
  disabling a component unregisters its settings card.
- **Fix**: stop a started client when `enabled` turns off; restart only a
  component that `begin()` wanted and `shutdown()` did not cancel.
- **Refs**: BUG-71, whose flags make the fix a few lines; BUG-67.
...
| **Total** | **198 items** | | **0C, 0H, 26M, 57L** (128 resolved) |
