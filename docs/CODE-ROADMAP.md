...
| MQTT, System, HomeAssistant, NTP, OTA, WebUI | BUG-72, BUG-73 to BUG-86 (filed) | **PR #93** — 2026-09-25: a client id stored empty connected as an empty id, filed from a consumer's production panel (BUG-73, HIGH), and the sweep behind it found thirteen more settings that a card or storage set after `begin()` and that never reached the session, never came back after a reboot, or came back over a choice. A native project with every component now proves the round trip through a reboot. Native 1 270 → 1 304 locally; the WROOM-32D each side of BUG-73 and BUG-74, and a card rename reopening the session |
| HomeAssistant, MQTT, OTA | BUG-68, CI-20, BUG-87 to BUG-90 (filed), TEST-13 (filed) | **PR pending** — 2026-09-26: a discovery document states a payload only where the caller changed Home Assistant's default — 45 characters off every entity, up to 60 more on a switch or a light, four of the restatements found in Home Assistant's schemas rather than in the filing; the shipping `esp32dev` build compiles without a warning, eleven inline variables where four were filed. Native HomeAssistant 167 → 176. The next release owes a CHANGELOG line: the keys leave every document, and a button honours `useAvailability` |
| WebUI, NTP, Core (HAL) | BUG-69, BUG-71 (filed), BUG-72 (filed) | **PR #91** — 2026-09-25: the WebUI's server and the SNTP client were the two other places an ESP32 reached lwIP before any network interface existed; both boot-looped the WROOM-32D alone, both now ask `HAL::canOpenServer()` — moved to Core's platform HAL — and start from `loop()`. 14 native cases, twelve removal checks, one probe run each side of each fix; three CHANGELOG lines owed (see the entries). The review filed BUG-72 |
...
**The eighth ships as v2.7.0** (2026-09-24): BUG-63, BUG-13 and BUG-62, BUG-64
(the platform's own log lines in the console), BUG-65, TEST-12, CI-11, CI-19 and
BUG-67. A minor, because BUG-64 adds public capability. Its entry omitted that
the ESP32 console history had tripled, and added it the next day.

**The ninth ships as v2.7.1** (2026-09-25), corrective like v2.6.1: BUG-70 brings
the console history 2.7.0 had tripled back to 64 lines, the cause of a production
ESP32 settling 23 KB lower; BUG-69 and BUG-71 ride with it. **Five components take
a patch** — Core 1.11.1, NTP 1.4.1, RemoteConsole 1.7.1, WebUI 1.10.1, Wifi 1.6.1.
Native 1 270, read from CI. The release gate is not claimed: 26 MEDIUM open.

**The tenth ships as v2.8.0** (2026-09-25), corrective and a minor: BUG-73, a
HIGH filed from a consumer's production panel, with BUG-72 and BUG-74 to BUG-86,
settings a card or storage set after `begin()` that never reached the session or
never came back. A minor because it adds persisted keys and
`OTAWebUI::setConfigSaveCallback()`, and because `SystemConfig`'s NTP fields now
take effect. **Seven components move** — MQTT 1.8.0, OTA 1.10.0, System 1.9.0
(minor); HomeAssistant 2.4.1, NTP 1.4.2, WebUI 1.10.2, Storage 1.6.1 (patch).
Native 1 304 over 15 projects, read from CI. The release gate is not claimed: 26
MEDIUM open.
...
- **Problem**: `begin()` hands **every** message the shared MQTT client receives
  ...
  never addressed by.
- **Measured** on a consumer's bench against DomoticsCore 2.6.1, whose documented
...
- **Refs**: filed from a production alarm panel, which carries its own
  command topic by design; MEM-2, which made this path
...
- **Refs**: filed from a production alarm panel. See also BUG-64,
...
- **Refs**: filed from a production alarm panel, which has no serial
  line. See BUG-63, filed the same session. Probes:
...
### BUG-69 — WebUI: the other listening socket does not ask whether it can open [MEDIUM] — **DONE (2026-09-25)**

- **Files**: `WebUI/WebServerManager.h` (`start()`), `WebUI.h` (`loop()`), against
  `HAL::canOpenServer()`.
- **Problem**: BUG-67 was a component opening a listening socket in `begin()`
  before lwIP existed. The WebUI's `AsyncWebServer` was the other such site, and
  still called `begin()` unconditionally. `System.h` composes WiFi first, which
  hid it the way the console was hidden.
- **Measured** on the WROOM-32D, WebUI alone and no WiFi component: a boot loop
  on `assert failed: tcpip_api_call … (Invalid mbox)` right after the SSE line.
- **Fixed**: the manager asks in `start()`, logs the deferral, and `poll()`,
  called from the component's `loop()`, opens once an interface exists; `stop()`
  clears the wish, so nothing reopens after `shutdown()`, and never ends a server
  it did not begin. The
  predicate moved from the WiFi HAL to Core's platform HAL, same name: neither
  WebUI nor NTP depends on WiFi, and it is a fact about the platform's stack.
- **Verification**: a new native suite (`test_webui_server_gate`, 5 cases) red
  first; four removal checks each turn a case red — the HAL gate, the `poll()`
  call, the wish `stop()` clears, the guard on `end()`. On the board, same probe:
  deferred at 407 ms, the probe creates the station interface at 8 s, `Web server
  listening on port 80` at 8 106 ms, no reset. FullStack unchanged on the same
  board: listening from `begin()`, HTTP 200.
- **Owed at the next release**: on ESP32 with no interface yet, the WebUI is not
  listening when `begin()` returns; `HAL::canOpenServer()` now comes from Core's
  `Platform_HAL.h` (still reachable through `WiFiServer_HAL.h`).
- **Refs**: BUG-67, the same defect at the console; BUG-71, found by this lot's
  first board run; the probe `tools/on-device/probes/webui-no-netif`.

### BUG-71 — NTP: the SNTP client started before any network interface aborts an ESP32 [MEDIUM] — **DONE (2026-09-25, filed and fixed the same day)**

- **Files**: `NTP.h` (`begin()` → `HAL::NTP::init()` → `esp_sntp_init()`).
- **Problem**: the question BUG-69's entry left unverified. `begin()` started the
  SNTP client unconditionally; on ESP32 that reaches lwIP, which does not exist
  before the first network interface.
- **Measured** on the WROOM-32D, NTP alone and no WiFi component: a boot loop on
  `assert failed: tcpip_callback … (Invalid mbox)` after the server list. Hidden
  like BUG-69 by every shipped composition registering WiFi first.
- **Fixed**: same shape. `begin()` sets the timezone and starts the client only
  if `HAL::canOpenServer()` allows it, else logs the deferral; `loop()` starts a
  deferred client, never after `shutdown()`. `syncNow()` refuses while deferred —
  `forceSync()` reaches the same stack. `setConfig()`'s restart clears the started
  flag before `begin()`, or a running client would stay stopped. `stop()` is only
  called on a client that started: on the fork's core 3.x it reaches lwIP too.
- **Verification**: a new native suite (`test_ntp_client_gate`, 9 cases), three
  red on the unfixed code; eight removal checks each turn a case red — among them
  the restart flag and the three `stop()` guards. On the board: deferred at
  423 ms, `SNTP client started via HAL` at 8 103 ms, no reset; FullStack syncs as
  before.
- **Owed at the next release**: `syncNow()` returns `false` until the client has
  started, which on ESP32 means until a network interface exists.
- **Refs**: BUG-69 and BUG-67, the same defect at the two servers; BUG-72, found
  by this lot's review.

### BUG-72 — NTP: `setConfig()` follows `enabled` one way only [LOW] — **DONE (2026-09-25, in BUG-73's lot)**

- **Files**: `NTP.h` (`setConfig()`), reached from `NTPWebUI.h`'s `enabled` field.
- **Problem**: the restart ran only when the new config was enabled, so turning
  `enabled` off stopped nothing: the SNTP client kept setting the clock until the
  next boot. And the restart ran whatever `shutdown()` had said, so an application
  calling `setConfig()` on a stopped component started it again.
- **Fixed**: the restart is gated on being between `begin()` and `shutdown()`, not
  on `enabled`; `begin()` then leaves a disabled client stopped. The explicit
  "disable stops" branch first written for it was dropped when its removal check
  stayed green: the gate alone carries it.
- **Verification**: four cases in `test_ntp_client_gate`, three red on the old gate.
- **Refs**: BUG-71, whose flags made this a few lines; BUG-67.

### BUG-73 — MQTT: a client id stored empty connects as an empty id [HIGH] — **DONE (2026-09-25, filed and fixed the same day)**

- **Files**: `MQTT_impl.h` (`setConfig()`), `SystemPersistence.h` (`loadMQTTConfig()`),
  `MQTTWebUI.h` (`client_id`).
- **Problem**: the settings card accepts an empty Client ID and stores it; the
  loader prefers the stored `""` over the default; `setConfig()` copies it, and
  only the constructor generated an id. PubSubClient then sends an empty id, and
  the broker assigns a random one at every connection.
- **Consequence**: a reconnection never takes over the previous session. The dead
  one lives until its keep-alive expires, then the broker publishes its retained
  `offline` will — after the new session's `online`. Home Assistant shows every
  entity unavailable indefinitely and drops the commands sent to them. Filed from a
  consumer's production alarm panel, which could not be disarmed from Home
  Assistant after an abrupt reboot.
- **Measured** on the WROOM-32D, `mqtt_clientid` emptied through the card: on
  2.7.1 two resets connect as `auto-B4CFF0B3…` then `auto-53A33987…`; with the fix,
  as `ESP32-880c6bf7a608` both times, the broker logging `session taken over` and
  the new `online` 0.5 s after the old will.
- **Fixed**: `setConfig()` normalises as the constructor does — an empty id means
  the generated one — and the loader keeps the configured id over a stored empty
  one (BUG-82).
- **Verification**: `test_mqtt_config_apply`, red first; one removal check.
- **Refs**: BUG-74, which set that panel's `k15`; BUG-75 to BUG-86, the same
  family found by this lot's sweep.

### BUG-74 — MQTT: keep-alive is applied in `begin()` only [MEDIUM] — **DONE (2026-09-25)**

- **Files**: `MQTT_impl.h` (`begin()`, `connectInternal()`).
- **Problem**: `setKeepAlive()` ran in `begin()`, after its two early returns (no
  broker, disabled), and System loads the stored config after `core.begin()`. A
  broker that comes from storage left PubSubClient on its own 15 s; a keep-alive
  changed later was never applied.
- **Measured**: 2.7.1 on the WROOM-32D, broker from NVS: `k15` against a
  configured 60; with the fix, `k60`.
- **Fixed**: set at each connection, from the config as it is then.
- **Verification**: two cases, red first (`Expected 60 Was 15`); one removal check.

### BUG-75 — MQTT: the default Last Will topic does not follow a new client id [MEDIUM] — **DONE (2026-09-25)**

- **Files**: `MQTT_impl.h` (`normalizeConfig()`), `HomeAssistant.h` (`mqtt/connected`).
- **Problem**: `{clientId}/status` was derived in the constructor only. A client id
  loaded from storage or set in the card left the will on the compiled id's topic
  while telemetry moved to the new prefix, and Home Assistant, which adopts the
  will topic, did not follow a change made at runtime.
- **Fixed**: a will topic that was the previous id's default follows the new id; a
  named one stays. Home Assistant reconciles with the will at every connection.
- **Verification**: three MQTT cases and one Home Assistant case, red first; two
  removal checks. On the board a rename in the card moved `online` to
  `bench-renamed/status`.

### BUG-76 — MQTT: session settings changed while connected wait for the next reconnection [MEDIUM] — **DONE (2026-09-25)**

- **Files**: `MQTT_impl.h` (`setConfig()`, `loop()`).
- **Problem**: broker, port, credentials, client id, TLS, keep-alive and the will
  go out in CONNECT. The card answered success and the live session kept the old
  values until the link next dropped, possibly days later.
- **Fixed**: a change to one of them while connected — or while a connection
  attempt blocks, the review's catch — marks the session, and the next `loop()`
  closes it cleanly and reopens it, from the loop task rather than the web handler
  that called `setConfig()`. Other fields reopen nothing.
- **Verification**: two cases, red first; two removal checks. On the WROOM-32D a
  card rename reconnected as the new id within the second, with no will fired.

### BUG-77 — MQTT: an empty Last Will topic reaches CONNECT [MEDIUM] — **DONE (2026-09-25)**

- **Files**: `MQTTWebUI.h` (`lwt_topic`), `MQTT_impl.h` (`normalizeConfig()`).
- **Problem**: the card accepted an empty `lwt_topic` and the client sent a will
  flag with an empty topic, which MQTT forbids: the broker refuses the CONNECT and
  the device retries until a reboot.
- **Fixed**: an empty will topic means `{clientId}/status`, in `setConfig()` as in
  the constructor. Home Assistant's "empty will topic" case, which pinned the old
  refusal to adopt it, now pins that availability follows the default.
- **Verification**: one case, red first; one removal check.

### BUG-78 — MQTT: `lwtQoS` is clamped by the constructor only [LOW] — **DONE (2026-09-25)**

- **Problem**: `setConfig()` accepted 3, which sets both will-QoS bits and makes a
  malformed CONNECT. **Fixed** in the shared normalisation. One case, red first.

### BUG-79 — MQTT: `useTLS` changed after construction is never applied [LOW] — **DONE (2026-09-25)**

- **Problem**: the transport is chosen when the client is built. The card stored
  the flag and showed TLS on while the link stayed plaintext.
- **Fixed**: a changed flag rebuilds the client in `loop()` before the next
  connection, which BUG-76's reopen triggers; not in `connect()`, which the card can
  call from the web task. One case, red first; one removal check. Not measured on a
  board: the bench broker has no TLS listener, and neither HAL client is given a CA,
  so turning TLS on from the card keeps a device off a TLS broker until it is
  turned back off — reachable now, where the field used to do nothing.

### BUG-80 — System: a rename from the SystemInfo card is reverted by a WebUI settings save [MEDIUM] — **DONE (2026-09-25)**

- **Files**: `SystemWebUISetup.h` (the rename callback).
- **Problem**: the rename updated Storage, `SystemConfig` and SystemInfo but not
  the WebUI's copy, so the page header kept the old name, and the next save of any
  WebUI setting wrote the old name back to `device_name`: the rename was lost at
  the next boot, without a message.
- **Fixed**: the rename updates the WebUI's copy too.
- **Verification**: `test_full_composition`, red first; one removal check.

### BUG-81 — HomeAssistant: the card accepts an empty node id, prefix or device name [MEDIUM] — **DONE (2026-09-25)**

- **Problem**: stored, the empty value won at every boot: no prefix means no
  discovery, no node id an empty topic level. Recorded only as a residual inside
  BUG-31. **Fixed**: refused with `This field cannot be empty`; the descriptive
  fields may still be cleared. One case, red first; one removal check.

### BUG-82 — System: a stored empty value overrides what the firmware was built with [MEDIUM] — **DONE (2026-09-25)**

- **Files**: `SystemPersistence.h` (every `load*`).
- **Problem**: `getString(key, current)` returns a stored `""`, so an emptied field
  — or a firmware reflashed with a new configured broker — was overridden at every
  boot. The MQTT side is where BUG-73's empty id came from.
- **Fixed**: `storedOr()` keeps the configured value when the stored one is empty,
  only where empty can mean nothing but the default (client id, will topic, the
  Home Assistant identity, the WebUI fields, the timezone, the device name). The
  review narrowed a first version that also covered the broker, the credentials and
  the descriptive fields — where empty is a choice, and a cleared username for an
  anonymous broker came back at every boot.
- **Verification**: four cases, red first — once the stub was made to persist: it
  kept entries per instance, so "the next boot" always started empty and two cases
  passed on the unfixed code. Three removal checks.

### BUG-83 — System: `SystemConfig::ntpServer` and `ntpTimezone` never reach NTP [LOW] — **DONE (2026-09-25)**

- **Problem**: documented, and read only by a config test; `registerNTPComponent()`
  built NTP with its own defaults. **Fixed** through `systemNtpConfig()`; the
  `"UTC"` default, which has no offset, becomes `"UTC0"`. Two cases, one red first.

### BUG-84 — OTA: a check interval changed after `begin()` is not rescheduled [LOW] — **DONE (2026-09-25)**

- **Problem**: switched on from 0 it fired at once; shortened, it waited out the
  old one. **Fixed**: `setConfig()` reschedules when the interval changes. Two
  cases, red first; one removal check.

### BUG-85 — System: some card edits are applied and never persisted [LOW] — **DONE (2026-09-25)**

- **Problem**: MQTT's TLS and Last Will fields, Home Assistant's manufacturer,
  model and area, and OTA's URL and auto-reboot were lost at reboot; OTA's card had
  no save callback at all.
- **Fixed**: eight keys, their saves and their loads; `OTAWebUI::setConfigSaveCallback()`.
  The MQTT card now saves the configuration the component applied: saving its own
  copy stored a will topic from before the rename, which came back after two
  renames and a reboot (the review's find).
- **Verification**: four cases, red first; four removal checks.

### BUG-86 — WebUI: the card accepts an empty theme, colour or username [LOW] — **DONE (2026-09-25)**

- **Problem**: stored, the empty value won at every boot. **Fixed**: the theme must
  be `dark`, `light` or `auto`; an empty colour or username is refused. One case,
  red first; three removal checks.

### BUG-87 — HomeAssistant: every brightness Home Assistant sends is read as 255 [LOW] — **NEW (2026-09-26, filed by BUG-68's lot)**

- **Files**: `HALight.h` (`handleCommand()`), against the discovery document the
  same class writes.
- **Problem**: the document declares Home Assistant's default schema with
  `on_cmd_type: brightness` and `bri_cmd_t` equal to `cmd_t`. That schema sends
  `OFF`, or the brightness as a bare number, to the command topic, never JSON.
  `handleCommand()` parses JSON first, and `"128"` parses: a number, with no
  `state` and no `brightness` key, so both take their defaults. On a board that
  is `state == true`, `brightness == 255`, for `"128"` and for `"0"` alike. The
  native harness shows `state == false` instead, which is TEST-13, not this.
- **Consequence**: `HALight::brightness` reads 255 after every brightness change
  made from Home Assistant, and a light dimmed to 0 reads on. The raw payload
  still reaches the application through `ha/command`, which is why nothing in the
  tree noticed.
- **LOW, argued** as BUG-61 was: nothing in the repository reads those two
  fields outside the command path. The reference documents them as the current
  state, which is what a consumer would read.
- **Fix**: read a bare integer as the brightness, `state` following it; decide
  whether the JSON branch should stay, since no schema this class declares
  sends JSON.

### BUG-88 — MQTT: `useTLS` connects to no broker, and nothing public can fix that [LOW] — **NEW (2026-09-26, from BUG-79's residue)**

- **Files**: `MQTT_ESP32.h`, `MQTT_ESP8266.h` (`WiFiClientSecure`, given neither
  a CA nor an insecure mode), `MQTT.h` (`mqttClient` is private).
- **Problem**: both HAL clients build a `WiFiClientSecure` and hand it nothing a
  handshake needs, and the component offers no way to reach it. The MQTT
  reference says certificate configuration "must be done at the HAL level" —
  which an application cannot do.
- **Consequence**: since BUG-79 the settings card really switches the transport,
  so a device switched to TLS from the card leaves the broker and stays off it
  until the field is turned back off. Before 2.8.0 the same flag at construction
  was just as dead; nobody had tried it.
- **LOW, argued**: off by default, reversible from the same card, and announced
  in 2.8.0's notes. Raise it if a consumer is found relying on TLS.
- **Fix**: a decision — a CA (or fingerprint) in `MQTTConfig` passed to the HAL
  client, or removing the field from the card until one exists. Not measured on
  a board: the bench broker has no TLS listener.

### BUG-89 — HomeAssistant: the alarm panel writes `cod_*_req: true`, which is Home Assistant's default [LOW] — **NEW (2026-09-26, by BUG-68's review)**

- **Files**: `HAAlarmControlPanel.h` (`cod_arm_req`, `cod_dis_req`, always;
  `cod_trig_req` with the `Trigger` feature).
- **Problem**: BUG-63 wrote the requirement keys always, because Home Assistant
  reads an absent one as `true` and this component defaults to `false`. Writing
  `false` is required; writing `true` restates Home Assistant's default, up to
  52 characters on a panel that requires a code for everything.
- **Consequence**: that panel is exactly the shape that crosses the 699-character
  field: the six-mode panel with a code and a 32-character node id is 741, refused
  whole. Whether dropping the `true` keys brings it under is to be measured, not
  assumed: that panel's requirement flags decide how many are `true`.
- **Fix**: write a requirement key only when it is `false`; keep the vocabulary
  pin as the model.

### BUG-90 — HomeAssistant: a button drops its entity category, value template and attributes topic [LOW] — **NEW (2026-09-26, by BUG-68's review)**

- **Files**: `HAButton.h` (`buildDiscoveryPayload()` does not call the base).
- **Problem**: the button writes its own document, without `stat_t`, and with it
  drops `ent_cat`, `val_tpl` and `json_attr_t`, which the base emits when set.
- **Consequence**: a button marked `entityCategory = "config"` or `"diagnostic"`
  lands among the device's controls; nothing says the field was ignored.
- **Fix**: share the base's optional fields rather than copying the block, as the
  availability block now is.

### BUG-68 — HomeAssistant: every entity's document still restates Home Assistant's own payloads [MEDIUM] — **DONE (2026-09-26)**
...
- **Fixed**: read against Home Assistant's own MQTT schemas rather than its
  documentation, which found four restatements the filing had not listed:
  `stat_on`/`stat_off` on the switch (Home Assistant reads their absence as the
  command payload), `pl_prs` on the button, and on the light `bri_scl: 255`
  (the default) and `brightness: true`, a key of the JSON schema that the default
  schema strips before reading. A document now states a payload only where the
  caller changed it; the defaults are `HAPayload` constants, and the Last Will's
  default and `setAvailable()` are pinned to them, so document and will cannot
  drift apart unseen. `HAButton` loses its copy of the availability block and with
  it one divergence: it ignored `useAvailability`. It still builds its own
  document and drops the other optional base fields, which is BUG-90. The alarm
  panel's always-written `cod_*_req: true` is the same waste on requirement
  keys rather than payloads, left as BUG-89. A CHANGELOG line is owed: the keys
  leave every document, and a button with `useAvailability = false` no longer
  publishes `avty_t`.
- **Cost recovered per document**: 45 characters on every entity; 60 more on a
  default switch, 60 on a light, 28 on a binary sensor, 17 on a button. The alarm
...
- **Files**: `ArduinoJsonString.h:32` (`canConvertFromJson` accepts `isNull()`),
  the stub `String` every native suite uses.
- **Problem**: `doc[k] | String(dflt)` on a missing key returns `""` on the host
  and `dflt` on a board, where ArduinoJson's own `String` support refuses a null.
- **Consequence**: a native test of that idiom runs a different path from the
  device. Found by BUG-87, whose measured `state` was false natively and true on
  the target's logic; any suite asserting a default read this way pins the
  host's behaviour, not the product's.
- **Fix**: refuse null as the library does, then read what moves; every
  `| String(` in the tree is a candidate.
...
### BUG-70 — 2.7.0's steady-state heap footprint is about 23 KB larger than 2.6.1's [LOW] — **DONE (2026-09-25)**

- **Files**: `Platform_ESP32.h` (`DOMOTICS_LOG_BUFFER_SIZE`), `RemoteConsole.h` (`log()`).
- **Filed as a leak, corrected to a footprint**: a production ESP32's allocatable
  heap fell -639 B/h after the upgrade, then flattened (-231, -196, -112, 0 B/h)
  at 85 064 bytes against 107-109 KB on 2.6.1. `allocation_failures` stayed 0.
- **Cause, in this repository's own record**: BUG-64's lot moved the console
  history from **50 to 150 lines on ESP32** (5 to 20 on ESP8266) and wrote *"23 KB
  when full"* into its entry without arguing the number; 2.7.0's CHANGELOG never
  mentioned it. Messages are taken as lines arrive: the settling curve.
- **Measured through the component, allocatable heap, WROOM-32D**:
  ```
  line      50 (2.6.1)   150 (2.7.0)   64 (now)
  40 ch        5 784        19 856        6 672
  120 ch       9 776        31 856       11 796
  ```
  150 − 50 at 120 characters is **22 080 B**, the production gap. The 129th line
  doubled the vector to 256 slots — 106 dead, 4.2 KB — because nothing reserved.
- **Fixed**: ESP32 default **64** (platform lines now share the history with ours),
  ESP8266 stays 20; slots reserved once, at the cap, by the first line, which also
  covers `clearBuffer()`.
- **The review found a ratchet the first fix did not see**: an overwritten slot
  kept its larger buffer, so a history tends to *cap × the longest line each slot
  ever held*. Moving the entry in is not enough on ESP32 — core 2.0.17's
  `String::move` copies into the destination's buffer when it fits — so the slot is
  destroyed and rebuilt. After 64 short lines over 64 long ones the WROOM holds
  5 656 B; with a plain move, 11 796 — nothing returned. ESP8266's move frees the
  destination, so it passes either way (3 368 → 1 448).
- **Verification**: native, the reservation twice (`Expected 100 Was 128`,
  `Expected 100 Was 1`) and the release on clear; on both boards, the history's
  figures above; and a `static_assert` that holds the default's worst case — every
  line a full `DLOG_*` line — under 20 KB, compiled by the required `Build
  on-device suites` job. It fails at 150, so it stops a release the way nothing in
  2.7.0 did; the board case alone could not, since CI builds that suite and never
  runs it. 72 → 75 native.
- **Owed at the next release**: the ESP32 default goes 150 → 64; the slots are
  taken at the first line rather than grown; an overwritten line no longer keeps
  its predecessor's buffer. The 2.7.0 entry already says what it omitted, with the
  workaround a deployed device has today.
- **Refs**: filed from a production ESP32 running 2.7.0; BUG-64, whose lot made the
  change; the probe `tools/on-device/probes/console-ring-cost`.
...
### CI-20 — the shipping ESP32 build compiles C++17 inline variables as a GNU extension [LOW] — **DONE (2026-09-26)**
...
- **Refs**: TEST-12, whose lot made the ESP8266 and ESP32 test builds silent and
  left these standing.
- **Fixed**: there were eleven, not four: `OTAEvents.h` carries six and
  `Update_ESP32.h` one, which the filing's count missed. The event names become
  `static constexpr`, the form `Events.h` and `AlarmPanelCommand` already use.
  BUG-12 had chosen `inline` because the EventBus would compare topic pointers
  across translation units; it does not — it keys its subscriptions by `String`
  value — so a copy per unit costs nothing. The two mutable ones, BUG-7's
  `MQTTComponent::instance` and BUG-20's OTA byte counter, become a local static
  behind an inline accessor, which C++14 guarantees is one object.
  `FullStack` `esp32dev`: 7 warnings after the first two headers, 0 after all
  four, RAM and flash unchanged to the byte. `Update_ESP8266.h` keeps its two:
  that build is `gnu++17`, where they are standard. `esp32dev` stays at
  `gnu++14`.
...
| **Total** | **217 items** | | **0C, 0H, 25M, 60L** (145 resolved) |
